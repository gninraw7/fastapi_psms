-- DDL_20260203_Fix_CompanyCd_PK_step2.sql
-- 이전 실행 중단 이후 잔여 변경 적용

SELECT company_cd INTO @company_cd FROM companies ORDER BY company_cd LIMIT 1;
SET FOREIGN_KEY_CHECKS = 0;

-- 1) project_sales: company_cd 추가 + PK/FK
...
-- 2) tbl_file_storage: company_cd 추가 + PK
...
-- 18) board_notices FK 추가 (users 인덱스 확보 후)
...
-- 19) project_sales FK 추가 (projects PK 변경 후)
ALTER TABLE `project_sales`
