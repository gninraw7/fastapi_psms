...
def _is_field_in_use(db: Session, field_code: str) -> bool:
    queries = [
        "SELECT COUNT(*) AS cnt FROM projects WHERE field_code = :code",
        "SELECT COUNT(*) AS cnt FROM sales_plan_line WHERE field_code = :code",
        "SELECT COUNT(*) AS cnt FROM sales_actual_line WHERE field_code = :code",
    ]
    for q in queries:
        try:
            cnt = db.execute(text(q), {"code": field_code}).fetchone().cnt
            if cnt and cnt > 0:
                return True
        except Exception:
            continue
    ...
    try:
        ...
        for item in items:
            ...
            if row_stat == "N":
                ...
                exists = db.execute(
                    text("SELECT 1 FROM industry_fields WHERE field_code = :code"),
                    {"code": code}
                ).fetchone()
                if exists:
                ...
                db.execute(
                    text("""
                        ...
                        (field_code, field_name, org_desc, facility_desc, sort_order, is_use)
                        VALUES (:code, :name, :org_desc, :facility_desc, :sort_order, :is_use)
                    """),
                    {
                        "code": code,
                        "name": name,
                ...
                db.execute(
                    text("DELETE FROM industry_fields WHERE field_code = :code"),
                    {"code": code},
                )
